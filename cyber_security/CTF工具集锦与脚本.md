...
| ★SQLMap        | 自动化 SQL 注入  | `sqlmap -u URL --dbssqlmap -u URL -D db -T tbl --dump--tamper=space2comment` 绕 WAF → 详见 [SQLMap 指南](web/SQLMap实战使用指南.md) | <https://github.com/sqlmapproject/sqlmap>     |
...
| ★AntSword（蚁剑） | WebShell 管理 | 连接一句话木马（填 URL + 密码）虚拟终端执行命令、文件管理 → 详见 [蚁剑指南](http/蚁剑实战使用指南.md)                                                               | <https://github.com/AntSwordProject/antSword> |
...
- SQL 注入：SQLMap ★ 自动化脱库，标准 GET/POST 注入直接跑；识别失败或非常规注入点（JSON/GraphQL）转手工注入，完整参数与 tamper 见 [SQLMap 指南](web/SQLMap实战使用指南.md)
- WebShell 管理：AntSword ★ 连一句话木马后可视化操作文件和终端，编码器/解码器机制灵活，完整用法见 [蚁剑指南](http/蚁剑实战使用指南.md)
...
> 完整参数、tamper 绕过 WAF、OS Shell 等进阶用法见 [SQLMap 实战使用指南](web/SQLMap实战使用指南.md)。
...
> 完整用法见 [蚁剑实战使用指南](http/蚁剑实战使用指南.md)，本节为速查。
...
> 题型全景、解题决策树、各子题型深度实战（隐写术、流量分析、压缩包取证、多媒体隐写、内存取证、OSINT）见 [Misc 实战使用指南](misc/Misc实战使用指南.md)。本节为日常高频用法速查。
...
| 工具              | 用途           | 关键用法                                                                                                                  | GitHub 地址                                        |
| --------------- | ------------ | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| ★CyberChef      | 编解码瑞士军刀      | 在线拖拽操作链（From Base64 → From Hex → ...）支持 Magic 自动识别 → 详见 [CyberChef 指南](crypto/CyberChef实战使用指南.md)                            | <https://github.com/gchq/CyberChef>              |
| ★Wireshark      | 流量分析         | 显示过滤器 `http contains "flag"`Follow → TCP Stream 看会话File → Export Objects 还原文件 → 详见 [Wireshark 指南](misc/Wireshark实战使用指南.md) [深度手册](misc/Wireshark深度使用手册.md) | <https://github.com/wireshark/wireshark>         |
...
- 流量分析：Wireshark ★ 看 pcap 必备，完整过滤器语法、文件还原、USB 取证见 [Wireshark 指南](misc/Wireshark实战使用指南.md)；高级抓包、深度协议解析、网络取证、Lua 解析器、自动化流水线见 [Wireshark 深度手册](misc/Wireshark深度使用手册.md)；批量提取字段用配套的 tshark 命令行
...
> 完整过滤器语法、文件还原、USB 键鼠取证、tshark 批处理见 [Wireshark 实战使用指南](misc/Wireshark实战使用指南.md)。高级抓包、TLS/无线深度解析、网络取证工作流、Lua 解析器、自动化流水线见 [Wireshark 深度使用手册](misc/Wireshark深度使用手册.md)。
...
> 完整操作分类、Magic 自动识别、Recipe 保存分享、Registers/Fork 进阶、CTF 各题型实战见 [CyberChef 实战使用指南](crypto/CyberChef实战使用指南.md)。本节为日常高频用法速查。
...
| 工具           | 用途            | 关键用法                                                                       | GitHub 地址                                      |
| ------------ | ------------- | -------------------------------------------------------------------------- | ---------------------------------------------- |
| ★CyberChef   | 编解码/哈希/加密     | 在线拖拽操作链Hash 模块算 MD5/SHA、Magic 自动识别 → 详见 [CyberChef 指南](crypto/CyberChef实战使用指南.md) | <https://github.com/gchq/CyberChef>            |
...
| 工具            | 用途           | 关键用法                                                            | GitHub 地址                                          |
| ------------- | ------------ | --------------------------------------------------------------- | -------------------------------------------------- |
| ★IDA Pro      | 静态反汇编/反编译    | 拖入二进制 → F5 看伪代码X 查交叉引用、N 重命名→ 详见 [IDA Pro 指南](reverse/IDA_Pro实战使用指南.md) | 商业软件                                               |
...
| ★GDB + pwndbg | Linux 动态调试   | `gdb ./binary` → `b *0x401234` → `r`vmmap/telescope/cyclic_find→ 详见 [GDB+pwndbg 指南](pwn/GDB+pwndbg实战使用指南.md) | <https://github.com/pwndbg/pwndbg>                 |
...
- 静态逆向：IDA Pro ★ 行业标准，F5 反编译质量最高、调试器内置，完整用法见 [IDA Pro 指南](reverse/IDA_Pro实战使用指南.md)；无授权或需多人协作用 Ghidra（开源免费），两者反编译器各有盲区，IDA 失败时换 Ghidra 试试
- 动态调试：Windows 程序用 x64dbg（GUI 友好）；Linux 程序用 GDB + pwndbg ★（pwndbg 增强后上下文清晰，PWN/逆向通用），完整用法见 [GDB+pwndbg 指南](pwn/GDB+pwndbg实战使用指南.md)；两者按目标平台选，不冲突
...
> 完整用法（安装配置、断点管理、内存操作、pwntools 配合、题型实战等）见 [GDB+pwndbg 实战使用指南](pwn/GDB+pwndbg实战使用指南.md)。
...
| 工具            | 用途               | 关键用法                                                                                                             | GitHub 地址                                     |
| ------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| ★pwntools     | Python PWN 框架    | `from pwn import *remote(ip,port)`/`process()`、`p64`/`ROP`/`fmtstr_payload`→ 详见 [pwntools 指南](pwn/pwntools实战使用指南.md) | <https://github.com/Gallopsled/pwntools>      |
| ★GDB + pwndbg | 动态调试             | `gdb ./binary` → `b *0x401234` → `r`vmmap/telescope/cyclic→ 详见 [GDB+pwndbg 指南](pwn/GDB+pwndbg实战使用指南.md)                            | <https://github.com/pwndbg/pwndbg>            |
...
- PWN 框架：pwntools ★ 必装，连接/打包/ROP/格式化字符串全包，完整 API 与模板见 [pwntools 指南](pwn/pwntools实战使用指南.md)
- 动态调试：GDB + pwndbg ★ Linux PWN 标配，pwndbg 增强后寄存器/栈/反汇编一目了然，完整用法见 [GDB+pwndbg 指南](pwn/GDB+pwndbg实战使用指南.md)；Windows 程序换 x64dbg
...
> 完整 API（连接/IO/ELF/ROP/fmtstr/shellcraft/GDB 调试）与各题型实战模板见 [pwntools 实战使用指南](pwn/pwntools实战使用指南.md)。
...
> 完整 API 详解与各题型实战模板见 [pwntools 实战使用指南](pwn/pwntools实战使用指南.md)。本节为日常高频用法速查。
...
| base\_family.py       | ctf\_scripts/base\_family.py       | Base 全家族识别 + 递归解码 | `python3 base_family.py -r "ZmxhZ3s..."` |
...
| archive\_brute.py     | ctf\_scripts/archive\_brute.py     | 压缩包爆破/CRC32爆破/明文攻击 | `python3 archive_brute.py x.zip --dict pass.txt` |
...
| stego\_image.py       | ctf\_scripts/stego\_image.py       | 图片隐写：宽高修复/LSB/内嵌提取 | `python3 stego_image.py x.png --all`       |
| pcap\_forensics.py    | ctf\_scripts/pcap\_forensics.py    | CTF 流量取证：flag/HTTP/DNS/USB/TLS | `python3 pcap_forensics.py x.pcap --all`   |
| db\_exploit.py        | ctf\_scripts/db\_exploit.py       | Redis/MySQL/MongoDB 等未授权与爆破 | `python3 db_exploit.py redis 1.1.1.1 6379` |
