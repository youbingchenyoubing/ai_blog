...
| ---------------------- | --------- | ----- | ----- | -- | ------------------- |
| `qwen3.7-max`          | 阿里云通义千问   | ★★★★★ | ★★★★  | 低  | **首选**，中文公文/通知/合同生成 |
| `qwen3.7-plus`         | 阿里云通义千问   | ★★★★☆ | ★★★   | 极低 | 日常表格处理、简单文档         |
| `qwen3.6-flash`        | 阿里云通义千问   | ★★★★  | ★★    | 极低 | 轻量快速，简单任务           |
...
> 注：`qwen3.7-max`、`qwen3.7-plus`、`qwen3.6-flash` 均为阿里云百炼平台最新模型 ID，旧版 `qwen-max`/`qwen-plus` 仍可调用但已停止更新。
...
> 阿里云已于 2026 年将 DashScope 迁移至百炼平台。旧版 qwen-portal OAuth 已废弃，OpenClaw 中的提供商名称从 qwen 改为 bailian。旧域名 dashscope.aliyuncs.com 仍可使用，但推荐使用 WorkspaceId 专属域名（见 01-Windows安装配置卸载指南.md 第 4 节）。
...
```json5
{
  ...
  agents: {
    defaults: {
      model: {
        // 主模型：通义千问 3.7 Max，中文行政场景首选
        primary: "qwen3.7-max",
        // 回退模型：主模型不可用时依次尝试
        fallbacks: ["deepseek-chat", "glm-4-plus"],
      },
      models: {
        "qwen3.7-max": { alias: "千问3.7Max" },
        "qwen3.7-plus": { alias: "千问3.7Plus" },
        "qwen3.6-flash": { alias: "千问3.6Flash" },
  ...
  models: {
    providers: {
      // 阿里云百炼（千问系列）
      bailian: {
        // 旧版兼容域名仍可使用，推荐改用 WorkspaceId 专属域名
...
# 设置主模型
openclaw models set qwen3.7-max
...
| 策略         | 说明                                      |
| ---------- | --------------------------------------- |
| 日常用国产模型    | `qwen3.7-max`、`deepseek-chat` 价格远低于海外模型 |
| 简单任务用轻量模型  | `qwen3.7-plus` 处理简单表格和文档，成本极低           |
